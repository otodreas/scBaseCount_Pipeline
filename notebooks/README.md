...
**`utility/single_srx_cluster_validation.ipynb`**: Runs cluster validation for one local SRX without writing another h5ad. Each existing validation figure has its own cell and is displayed in the notebook.
...
**`analysis/analyze_atlas_DE.py`**: Thin exploratory wrapper around [`disease_markers`](../scripts/disease_markers/README.md) for the local 100k postprocessed sample. Writes adaptive shortlists under `output/atlas/v2/agent_analysis/`. For the full atlas, use [`pipelines/run_atlas_de_analysis.py`](../pipelines/run_atlas_de_analysis.py) instead.
