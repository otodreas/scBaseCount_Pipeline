...
%
\section*{Abstract}
The abundance of nucleotide sequence data has grown into the order of trillions of bases. As public archives grow in scale, so too does their fragmentation and complexity, meaning that disease-focused meta-analyses remain difficult to accomplish. We identified the need for producing atlas-scale datasets that both incorporate work spanning studies and are uniformly processed and targeted to disease areas. scBaseCount hosts uniformly processed single-cell RNA sequencing files, but those files do not contain usable clusters. Since clustering individual datasets is not feasible at this scale, we pulled lung datasets from scBaseCount, applied quality control, combined them into one atlas, corrected for study-level batch effects, and automated clustering by utilizing published cell-type labels as weak priors to inform clustering resolution. The atlas contains 9,307,963 cells from 1,410 Sequence Read Archive experiment accessions spanning 172 National Center for Biotechnology Information BioProject accessions. Follow-up checks showed that cells from different studies mixed more after batch correction, while cell-type structure was largely retained. The scale and integration of the clustered dataset presents a possibility of making novel cross-study findings, even across rare cell types, through downstream analysis.
...
A: UMAP of Leiden clusters at the Jaccard-selected resolution versus CZ CELLxGENE \texttt{cell_type} labels. B: Cluster and cell-type composition. C: Cluster count and Hungarian-matched Jaccard score across resolutions 0.1--2.0. D: Homogeneity, completeness, V-measure, NMI, and ARI versus resolution. The selected resolution was 0.8.}
\label{fig3}
\end{figure}
