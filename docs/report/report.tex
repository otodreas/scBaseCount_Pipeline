...
% Concatenating uniformly processed files is not integration. Technical effects that track laboratory, chemistry, or study can dominate a joint embedding and obscure shared cell types~\parencite{lueckenBenchmarkingAtlaslevelData2022}. Removing those effects requires a batch definition that groups related experiments rather than treating each file as its own batch, quality control that is stable across heterogeneous studies, and a way to choose cluster resolution when portal  labels already exist. Atlas-level benchmarks treat the trade-off between batch mixing and biological conservation as the quantity to measure~\parencite{lueckenBenchmarkingAtlaslevelData2022}.
