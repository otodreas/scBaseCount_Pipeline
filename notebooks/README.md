...
**`analysis/analyze_atlas_DE.ipynb`**: Loads a postprocessed atlas h5ad from [`pipelines/run_atlas_postprocessing.py`](../pipelines/run_atlas_postprocessing.py), joins [`disease_markers`](../scripts/disease_markers/README.md) labels, compares integrated clusters with preserved source labels, ranks cluster markers from the log-normalized HVG matrix, screens shared and disease-associated cluster candidates, and runs study-aware abundance plus same-study pseudobulk DE for supported candidates. Artifacts land under `output/atlas/v2/analysis/`.
