...
|---|----------|----------|----------|
| 1 | [`metadata.ipynb`](metadata.ipynb) | scBaseCount metadata Parquet files | `output/metadata/datasets.csv`, `output/metadata/accession_disease_categories.json`, `output/metadata/datasets_subset_qc.csv`, figures |
| 2 | [`study_context.ipynb`](study_context.ipynb) | `output/metadata/datasets.csv` | `output/context/contexts.jsonl` |
| 3 | [`clustering.ipynb`](clustering.ipynb) | `output/metadata/datasets.csv`, h5ad files | `output/clustering/data/{srx}_clustered.h5ad`, figures |
| 4 | [`cytetype.ipynb`](cytetype.ipynb) | `output/clustering/data/{srx}_clustered.h5ad`, `output/context/contexts.jsonl` | `output/cytetype/data/{srx}_cytetype_annotated.h5ad` |
...
**`cytetype.ipynb`** — Annotates cluster labels for a single sample using CyteType, driven by the study context string assembled from `output/context/contexts.jsonl`. Writes the annotated AnnData to `output/cytetype/data/{srx}_cytetype_annotated.h5ad`.
