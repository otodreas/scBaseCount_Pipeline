...
Inspect agreement between STATE labels (`obs["cell_type"]`) and CyteType labels on annotated h5ad files. Joins CyteOnto `cytescore_similarity`, maps CyteType cluster confidence, and can emit a pair-level summary CSV, a combined per-cell UMAP parquet, and an extremes table.
...
| `emitUmap` | `True` | Write combined `umap_cells.parquet` during fetch pass |
...
| `umap_cells.parquet` | Per-cell UMAP coordinates and labels (optional) |
...
UMAP export and extremes are independent: disable either with `--no-umap` or `--no-extremes`, or rebuild extremes alone with `--from-summary`.
